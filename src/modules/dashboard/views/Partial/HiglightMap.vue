...
async function loadMapData() {
    try {
        const mapData = await fetch('/json/indonesia.json')
            .then((res) => res.json());
        chartOptions.value.chart.map = mapData

        // Menambahkan data kecamatan setelah peta dimuat
        chartOptions.value.series[0].data = mapData.features.map((item: Record<string, any>) => {
            return {
                'slug': item.properties.slug,
                // value: 10,
                color: ['Jawa Barat', 'Aceh', 'Bali', 'Jambi', 'Jawa Timur', 'Kalimantan Barat', 'Gorontalo', 'Maluku'].includes(item.properties.state) ? '#03045e' : '#00b4d8',
