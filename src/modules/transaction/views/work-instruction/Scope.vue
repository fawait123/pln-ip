...
import type { CreateDocumentInterface, IPagination, ResponseDocumentInterface } from "@/types/GlobalType";
    ...
    if (e.file.length === 0) {
        ...
        try {
            await globalStore.deleteDocument([
                entity.document?.uuid as string,
            ]);
            refetchScope();
            ...
            <div class="max-w-full min-w-full">
                <Table label-create="Asset" :columns="ColumnsWorkInstruction" :entities="entitiesScope"
                    ...
                    <template #column_preview="{ entity }">
                        ...
                        <FormOnlyUploadFile v-else ref="attachment" :value="entity.document" :label="entity.asset"
                            :loading="is_loading_create" @save="(e) => saveFile(e, entity)" />
                    </template>
                </Table>
            </div>
        </div>
