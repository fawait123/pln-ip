...
import type { PartStdCreateInterface, PartStdCreateModelInterface, PartStdInterface } from "../types/PartStdType";
...
const options_part = ref<OptionType[]>([]); const modelValue = defineModel<boolean>({ default: false });
        ...
        if (props.selectedValue) {
            ...
            options_part.value = mergeArrays(
                [
                    {
                        value:
                            props.selectedValue?.part_uuid,
                        label:
                            props.selectedValue?.part?.name,
                    },
                ],
                new_data.filter(
                    (item) =>
...
<template>
    <Modal width="440" height="200" :showButtonClose="false"
        ...
        <form class="flex flex-col gap-4 max-h-[calc(100vh-200px)] overflow-y-auto mx-[-20px] px-5"
            ...
            <Select v-model="model.part_uuid" label="Part" options_label="label" options_value="value"
